# Скрипты для наполнения базы данных

## ⚙️ Настройка переменных окружения

Скрипты могут использовать переменные окружения для определения ID пользователя. Создайте файл `.env` в директории `server/` со следующим содержимым:

```env
# Server configuration
PORT=8000

# JWT Secret Key
JWT_SECRET=your_secret_key_here

# User ID for scripts (optional)
# Если не указан, скрипты будут работать с публичной схемой
# Если указан, скрипты будут работать с указанной схемой пользователя
USER_ID=1
```

### Приоритет получения USER_ID:
1. Из аргументов командной строки (если указаны)
2. Из переменной окружения `USER_ID` (из `.env` файла)
3. Публичная схема (если не указано)

---

## populate-test-workers.js

Скрипт для автоматического наполнения таблицы `workers` тестовыми данными.

### Описание

Скрипт создает 10 тестовых работников со случайными именами и фамилиями. Поле `position` может быть либо "worker", либо "itr" (определяется случайным образом).

### Использование

#### 1. Через npm script (рекомендуется)

```bash
npm run populate-workers
```

#### 2. Напрямую через node

```bash
node scripts/populate-test-workers.js
```

#### 3. Для конкретного пользователя (заполнит схему user_X)

```bash
node scripts/populate-test-workers.js 1
```

Где `1` - это ID пользователя. Скрипт заполнит таблицу `workers` в схеме `user_1`.

### Важно

- Скрипт использует `USER_ID` из `.env` файла, если не указан аргумент командной строки
- Без указания userId (ни в аргументах, ни в `.env`) скрипт заполнит данные в публичную схему (`public`)
- С указанием userId (например, `1`) скрипт заполнит данные в схему конкретного пользователя (`user_1`)
- Скрипт автоматически пропускает дубликаты, если они существуют
- После выполнения показывается статистика по распределению работников по позициям

### Примеры использования

#### 1. Заполнение с использованием USER_ID из .env

Создайте `.env` файл:
```env
USER_ID=1
```

Затем запустите:
```bash
npm run populate-workers
```

#### 2. Заполнение публичной схемы (без .env и аргументов)

```bash
npm run populate-workers
```

#### 3. Заполнение для конкретного пользователя через аргумент (игнорирует .env)

```bash
node scripts/populate-test-workers.js 1
```

### Пример вывода

```
Начинаю наполнение таблицы workers тестовыми данными...
Целевая схема: public

1. Добавлен: Александр Иванов (worker)
2. Добавлен: Дмитрий Петров (itr)
3. Добавлен: Максим Смирнов (worker)
...

✅ Успешно добавлено 10 из 10 тестовых работников в таблицу workers

📊 Распределение по позициям:
   worker: 6
   itr: 4

🎉 Скрипт завершен успешно!
```

---

## clear-workers.js

Скрипт для удаления всех записей из таблицы `workers`.

### Описание

Скрипт полностью очищает таблицу `workers` в указанной схеме (пользовательской или публичной).

### Использование

#### 1. Через npm script

```bash
npm run clear-workers
```

#### 2. Напрямую через node

```bash
node scripts/clear-workers.js
```

#### 3. Для конкретного пользователя (очистит схему user_X)

```bash
node scripts/clear-workers.js 1
```

Где `1` - это ID пользователя. Скрипт очистит таблицу `workers` в схеме `user_1`.

### ⚠️ Важно

- Скрипт использует `USER_ID` из `.env` файла, если не указан аргумент командной строки
- Без указания userId (ни в аргументах, ни в `.env`) скрипт удалит ВСЕ записи из публичной схемы (`public`)
- С указанием userId (например, `1`) скрипт удалит записи из схемы конкретного пользователя (`user_1`)
- **Это действие необратимо!**
- Скрипт показывает количество удаленных записей

### Примеры использования

#### 1. Очистка с использованием USER_ID из .env

Создайте `.env` файл:
```env
USER_ID=1
```

Затем запустите:
```bash
npm run clear-workers
```

#### 2. Очистка публичной схемы (без .env и аргументов)

```bash
npm run clear-workers
```

#### 3. Очистка для конкретного пользователя через аргумент (игнорирует .env)

```bash
node scripts/clear-workers.js 1
```

### Пример вывода

```
⚠️  ВНИМАНИЕ: Вы собираетесь удалить ВСЕ записи из таблицы workers для пользователя 1!
Схема: user_1
Это действие необратимо.

Начинаю очистку таблицы workers...
Целевая схема: user_1

Найдено записей: 10

✅ Успешно удалено 10 записей из таблицы workers в схеме user_1
📊 Оставшихся записей: 0

🎉 Скрипт завершен успешно!
```

---

## clear-parent-calculations.js

Скрипт для удаления всех записей из таблицы `parent_calculation` для указанного пользователя.

### Описание

Скрипт полностью очищает таблицу `parent_calculation` в указанной схеме пользователя. При удалении `parent_calculation` записи каскадно удаляются все связанные `calculation` записи и все их дочерние данные (specification_data, workers_data, itr_data, workers_tax_data, itr_tax_data) благодаря FOREIGN KEY с ON DELETE CASCADE.

### Использование

#### 1. Через npm script (рекомендуется)

```bash
npm run clear-parent-calculations
```

#### 2. Напрямую через node

```bash
node scripts/clear-parent-calculations.js 1
```

Где `1` - это ID пользователя. Скрипт очистит таблицу `parent_calculation` в схеме `user_1`.

#### 3. С использованием USER_ID из .env

Создайте `.env` файл:
```env
USER_ID=1
```

Затем запустите:
```bash
npm run clear-parent-calculations
```

### ⚠️ Важно

- Скрипт **обязательно требует** указания `USER_ID` (в аргументах или в `.env` файле)
- С указанием userId (например, `1`) скрипт удалит записи из схемы конкретного пользователя (`user_1`)
- **Это действие необратимо!**
- Все связанные расчеты (calculations) и их данные будут удалены каскадно
- Скрипт показывает количество удаленных записей (parent_calculation и связанных calculations)

### Примеры использования

#### 1. Очистка для пользователя 1 (через аргумент)
```bash
node scripts/clear-parent-calculations.js 1
```

#### 2. Очистка с использованием USER_ID из .env (через npm)
```bash
# В .env: USER_ID=1
npm run clear-parent-calculations
```

### Пример вывода

```
⚠️  ВНИМАНИЕ: Вы собираетесь удалить ВСЕ записи из таблицы parent_calculation для пользователя 1!
Схема: user_1
Это действие необратимо.

Начинаю очистку таблицы parent_calculation...
Целевая схема: user_1

Найдено записей parent_calculation: 5
Найдено связанных записей calculation: 15
(Все связанные calculations будут удалены каскадно)

✅ Успешно удалено 5 записей из таблицы parent_calculation
✅ Удалено 15 связанных расчетов (calculations)
📊 Оставшихся записей в parent_calculation: 0

🎉 Скрипт завершен успешно!
```

---

## clone-calculation.js

Скрипт для клонирования существующей `calculation` со всеми связанными данными.

### Описание

Скрипт копирует существующую `calculation` и все связанные с ней данные (specification_data, workers_data, itr_data, workers_tax_data, itr_tax_data, calculation_media_files). Также создает новую `parent_calculation` запись, если `calculation_type = 'plan'`.

### Использование

#### 1. Через npm script (с USER_ID в .env)

```bash
npm run clone-calculation
```

#### 2. Напрямую через node

```bash
node scripts/clone-calculation.js [sourceCalcId] [userId]
```

Где:
- `sourceCalcId` - ID исходной calculation для копирования (по умолчанию: 1)
- `userId` - ID пользователя (если не указан, берется из .env)

### ⚠️ Важно

- Скрипт **обязательно требует** указания `USER_ID` (в `.env` или через аргумент)
- Клонирует все данные из существующей calculation
- Создает новую `parent_calculation`, если `calculation_type = 'plan'`
- Для других типов использует существующий `parent_calculation_id`

### Примеры использования

#### Клонирование calculation ID=1 для пользователя из .env

Создайте `.env` файл:
```env
USER_ID=3
```

Затем запустите:
```bash
node scripts/clone-calculation.js 1
```

#### Клонирование для конкретного пользователя (игнорирует .env)

```bash
node scripts/clone-calculation.js 1 3
```

#### Клонирование другой calculation

```bash
node scripts/clone-calculation.js 2 1
```

### Пример вывода

```
Начинаю клонирование calculation ID=1 для пользователя 3
Целевая схема: user_3

✓ Найдена calculation: "Тестовый расчет"
  Тип: plan
  Parent ID: 1
✓ Загружены связанные данные:
  - specification_data: 5 записей
  - workers_data: 10 записей
  - itr_data: 3 записей
  - workers_tax_data: 12 записей
  - itr_tax_data: 8 записей
  - calculation_media_files: 3 файла

✓ Создана новая parent_calculation ID=2: "Тестовый расчет (копия 2024-01-15)"
✓ Создана новая calculation ID=2: "Тестовый расчет (копия)"
  ✓ Клонированы данные specification_data_table: 5 записей
  ✓ Клонированы данные workers_data_table: 10 записей
  ✓ Клонированы данные itr_data_table: 3 записей
  ✓ Клонированы данные workers_tax_data: 12 записей
  ✓ Клонированы данные itr_tax_data: 8 записей
  ✓ Клонированы медиа файлы: 3 файла

✅ Клонирование завершено успешно!
📊 Создана новая запись:
   - calculation ID: 2
   - parent_calculation ID: 2
   - title: "Тестовый расчет (копия)"

🎉 Скрипт завершен успешно!
```

---

## backup-calculations.js

Скрипт для создания полного бэкапа всех таблиц `parent_calculation` и всех связанных таблиц.

### Описание

Скрипт создает полный бэкап всех данных, связанных с расчетами:
- `parent_calculation` - родительские расчеты
- `calculation` - основные расчеты
- `specification_data` и `specification_data_table` - данные спецификаций
- `workers_data` и `workers_data_table` - данные по рабочим
- `itr_data` и `itr_data_table` - данные ИТР
- `workers_tax_data` - налоговые данные рабочих
- `itr_tax_data` - налоговые данные ИТР
- `calculation_media_files` - медиа файлы (PDF, JPG, PNG), прикрепленные к расчетам

Бэкап сохраняется в формате JSON в директории `server/backups/` с именем файла, содержащим дату и время создания.

### Использование

#### 1. Через npm script (рекомендуется)

```bash
npm run backup-calculations
```

#### 2. Напрямую через node

```bash
node scripts/backup-calculations.js [userId]
```

Где `userId` - это ID пользователя. Скрипт создаст бэкап для схемы `user_{userId}`.

#### 3. С использованием USER_ID из .env

Создайте `.env` файл:
```env
USER_ID=1
```

Затем запустите:
```bash
npm run backup-calculations
```

### ⚠️ Важно

- Скрипт **обязательно требует** указания `USER_ID` (в аргументах или в `.env` файле)
- Бэкап сохраняется в директории `server/backups/`
- Имя файла содержит дату и время: `backup_user_{userId}_{timestamp}.json`
- Бэкап включает все связанные данные в полном объеме
- Рекомендуется регулярно создавать бэкапы перед важными операциями

### Примеры использования

#### 1. Создание бэкапа для пользователя 1 (через аргумент)
```bash
node scripts/backup-calculations.js 1
```

#### 2. Создание бэкапа с использованием USER_ID из .env (через npm)
```bash
# В .env: USER_ID=1
npm run backup-calculations
```

### Пример вывода

```
Начинаю создание бэкапа для пользователя 1...
Целевая схема: user_1

✓ Найдено parent_calculation записей: 5

📦 Обработка parent_calculation ID=1: "Проект А"
  ✓ Найдено calculations: 3
📦 Обработка parent_calculation ID=2: "Проект Б"
  ✓ Найдено calculations: 2
...

✅ Бэкап успешно создан!
📁 Файл: /path/to/server/backups/backup_user_1_2024-01-15_14-30-45.json

📊 Статистика бэкапа:
   - parent_calculations: 5
   - calculations: 15
   - specification_data: 12
   - workers_data: 10
   - itr_data: 8
   - workers_tax_data: 45 записей
   - itr_tax_data: 32 записей
   - calculation_media_files: 8 файлов

🎉 Бэкап завершен успешно!
💾 Сохранен в: /path/to/server/backups/backup_user_1_2024-01-15_14-30-45.json
```

---

## restore-calculations.js

Скрипт для восстановления данных из бэкапа.

### Описание

Скрипт восстанавливает все данные из файла бэкапа, созданного скриптом `backup-calculations.js`. Восстанавливает все таблицы с сохранением связей между ними.

### Использование

#### 1. Через npm script

```bash
npm run restore-calculations <backupFilePath> [userId] [--clear]
```

#### 2. Напрямую через node

```bash
node scripts/restore-calculations.js <backupFilePath> [userId] [--clear]
```

Где:
- `backupFilePath` - путь к файлу бэкапа (обязательный)
- `userId` - ID пользователя (опционально, если не указан, берется из бэкапа или .env)
- `--clear` - флаг для очистки существующих данных перед восстановлением (опционально)

### ⚠️ Важно

- Скрипт **обязательно требует** указания пути к файлу бэкапа
- Если указан флаг `--clear`, все существующие данные будут удалены перед восстановлением
- **Действие с флагом `--clear` необратимо!**
- Скрипт автоматически определяет userId из бэкапа, если не указан явно
- Восстанавливаются все связи между таблицами
- Новые ID будут автоматически сгенерированы (старые ID не сохраняются)

### Примеры использования

#### 1. Восстановление из бэкапа (без очистки существующих данных)
```bash
node scripts/restore-calculations.js backups/backup_user_1_2024-01-15_14-30-45.json 1
```

#### 2. Восстановление с очисткой существующих данных
```bash
node scripts/restore-calculations.js backups/backup_user_1_2024-01-15_14-30-45.json 1 --clear
```

#### 3. Восстановление с использованием USER_ID из .env
```bash
# В .env: USER_ID=1
node scripts/restore-calculations.js backups/backup_user_1_2024-01-15_14-30-45.json
```

#### 4. Восстановление с относительным путем
```bash
node scripts/restore-calculations.js backups/backup_user_1_2024-01-15_14-30-45.json
```

### Пример вывода

```
⚠️  ВНИМАНИЕ: Будут удалены все существующие данные перед восстановлением!
Это действие необратимо.

Начинаю восстановление из бэкапа...
Файл бэкапа: /path/to/server/backups/backup_user_1_2024-01-15_14-30-45.json
Дата бэкапа: 2024-01-15T14:30:45.123Z
Целевая схема: user_1

⚠️  Очистка существующих данных...
✓ Удалено 3 существующих записей parent_calculation

📦 Восстановление parent_calculation: "Проект А"
  ✓ Создана parent_calculation ID=1
📦 Восстановление parent_calculation: "Проект Б"
  ✓ Создана parent_calculation ID=2
...

✅ Восстановление завершено успешно!

📊 Статистика восстановления:
   - parent_calculations: 5
   - calculations: 15
   - specification_data: 12
   - workers_data: 10
   - itr_data: 8
   - workers_tax_data: 45 записей
   - itr_tax_data: 32 записей
   - calculation_media_files: 8 файлов

🎉 Скрипт завершен успешно!
```

---

## Рекомендации по использованию бэкапов

1. **Регулярное создание бэкапов**: Рекомендуется создавать бэкапы перед важными операциями или регулярно (например, ежедневно)

2. **Хранение бэкапов**: 
   - Бэкапы сохраняются в `server/backups/`
   - Рекомендуется копировать бэкапы на внешние носители или в облачное хранилище
   - Можно настроить автоматическое удаление старых бэкапов

3. **Проверка бэкапов**: Периодически проверяйте целостность файлов бэкапа

4. **Восстановление**: 
   - Всегда тестируйте восстановление на тестовой среде перед восстановлением в продакшене
   - Используйте флаг `--clear` только если уверены, что хотите заменить все существующие данные

5. **Версионирование**: Файлы бэкапа содержат дату и время, что позволяет легко отслеживать версии

---

## create-media-files-table.js

Скрипт для создания таблицы `calculation_media_files` в указанной схеме (публичной или пользовательской).

### Описание

Скрипт создает таблицу для хранения медиа файлов (PDF, JPG, PNG), прикрепленных к расчетам. Таблица автоматически создается при создании схемы пользователя через `dbManager`, но этот скрипт полезен для:
- Создания таблицы в существующих схемах пользователей
- Создания таблицы в публичной схеме
- Миграции существующих баз данных

### Использование

#### 1. Через npm script (рекомендуется)

```bash
npm run create-media-files-table
```

Создаст таблицу в схеме, указанной в переменной окружения `USER_ID`, или в публичной схеме, если `USER_ID` не указан.

#### 2. Напрямую через node

```bash
node scripts/create-media-files-table.js
```

#### 3. Для конкретного пользователя (создаст в схеме user_X)

```bash
node scripts/create-media-files-table.js 1
```

Где `1` - это ID пользователя. Скрипт создаст таблицу в схеме `user_1`.

#### 4. Для публичной схемы

```bash
# Убедитесь, что USER_ID не установлен в .env, или не передавайте аргументы
node scripts/create-media-files-table.js
```

### ⚠️ Важно

- Скрипт использует `USER_ID` из `.env` файла, если не указан аргумент командной строки
- Без указания userId (ни в аргументах, ни в `.env`) скрипт создаст таблицу в публичной схеме (`public`)
- С указанием userId (например, `1`) скрипт создаст таблицу в схеме конкретного пользователя (`user_1`)
- Скрипт проверяет существование схемы пользователя перед созданием таблицы
- Скрипт проверяет существование таблицы `calculation` (необходима для внешнего ключа)
- Используется `CREATE TABLE IF NOT EXISTS`, поэтому скрипт безопасен для повторного запуска

### Примеры использования

#### 1. Создание в публичной схеме (без .env и аргументов)

```bash
npm run create-media-files-table
```

#### 2. Создание для пользователя 1 (через аргумент)

```bash
node scripts/create-media-files-table.js 1
```

#### 3. Создание с использованием USER_ID из .env

Создайте `.env` файл:
```env
USER_ID=1
```

Затем запустите:
```bash
npm run create-media-files-table
```

### Пример вывода

```
Создание таблицы calculation_media_files...
Целевая схема: user_1

✅ Таблица calculation_media_files успешно создана в схеме user_1

📋 Структура таблицы:
   1. id (integer) NOT NULL
   2. calculation_id (integer) NOT NULL
   3. file_name (character varying) NOT NULL
   4. file_type (character varying) NOT NULL
   5. file_size (integer) NOT NULL
   6. file_data (bytea) NOT NULL
   7. date_of_creation (timestamp with time zone)

🎉 Скрипт завершен успешно!
```

### Обработка ошибок

Скрипт проверяет и выводит понятные сообщения об ошибках:

- **Схема не существует**: Если указана схема пользователя, но она не была создана
- **Таблица calculation не существует**: Если отсутствует родительская таблица для внешнего ключа
- **Таблица уже существует**: Предупреждение, что таблица уже создана (не критично)

### Примечания

- Таблица автоматически создается при регистрации нового пользователя через `dbManager.createUserDatabase()`
- Для существующих пользователей используйте этот скрипт для добавления таблицы
- Внешний ключ настроен с каскадным удалением: при удалении расчета удаляются все связанные медиа файлы