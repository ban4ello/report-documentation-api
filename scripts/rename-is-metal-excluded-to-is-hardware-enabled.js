...
/**
 * Скрипт для переименования поля is_metal_excluded в is_hardware_enabled в таблице calculation
 * 
 * Использование:
 *   node scripts/rename-is-metal-excluded-to-is-hardware-enabled.js [userId]
 *   node scripts/rename-is-metal-excluded-to-is-hardware-enabled.js all  # для всех пользователей
 * 
...
async function renameIsMetalExcludedToIsHardwareEnabled(targetUserId = null) {
  const client = await pool.connect();

  try {
    // Если указано "all", обрабатываем все схемы пользователей
    if (targetUserId === 'all') {
      console.log('🔄 Переименование поля is_metal_excluded в is_hardware_enabled для всех пользователей...\n');
    ...
    console.log(`🔄 Переименование поля is_metal_excluded в is_hardware_enabled в таблице calculation...`);
    ...
    console.error('❌ Ошибка при переименовании поля:', error.message);
    console.error(error);
    process.exit(1);
  } finally {
    client.release();
...
async function alterSchema(client, schemaName) {
  try {
    ...
    // Проверяем, существует ли поле is_metal_excluded
    const oldColumnCheck = await client.query(
      `SELECT column_name 
       FROM information_schema.columns 
       WHERE table_schema = $1 AND table_name = 'calculation' AND column_name = 'is_metal_excluded'`,
      [schemaName]
    );

    // Проверяем, существует ли уже поле is_hardware_enabled
    const newColumnCheck = await client.query(
      `SELECT column_name 
       FROM information_schema.columns 
       WHERE table_schema = $1 AND table_name = 'calculation' AND column_name = 'is_hardware_enabled'`,
      [schemaName]
    );

    if (newColumnCheck.rows.length > 0) {
      console.log(`   ✓ Поле is_hardware_enabled уже существует в таблице calculation, пропускаем`);
      return;
    }

    if (oldColumnCheck.rows.length === 0) {
      console.log(`   ⚠️  Поле is_metal_excluded не найдено в таблице calculation, пропускаем`);
      return;
    }

    console.log(`   🔄 Переименование поля is_metal_excluded в is_hardware_enabled...`);

    // Переименовываем поле
    const alterQuery = `ALTER TABLE ${schemaName}.calculation RENAME COLUMN is_metal_excluded TO is_hardware_enabled`;
    await client.query(alterQuery);

    console.log(`   ✅ Поле успешно переименовано в is_hardware_enabled`);
...
// Запускаем скрипт
renameIsMetalExcludedToIsHardwareEnabled(userId).catch((error) => {
