...
/**
 * Скрипт для добавления поля is_metal_excluded в таблицу calculation
 * 
 * Использование:
 *   node scripts/add-is-metal-excluded.js [userId]
 *   node scripts/add-is-metal-excluded.js all  # для всех пользователей
 * 
...
async function addIsMetalExcluded(targetUserId = null) {
  const client = await pool.connect();

  try {
    // Если указано "all", обрабатываем все схемы пользователей
    if (targetUserId === 'all') {
      console.log('🔄 Добавление поля is_metal_excluded для всех пользователей...\n');
    ...
    console.log(`🔄 Добавление поля is_metal_excluded в таблицу calculation...`);
...
async function alterSchema(client, schemaName) {
  try {
    ...
    // Проверяем, существует ли уже поле is_metal_excluded
    ...
    if (columnCheck.rows.length > 0) {
      console.log(`   ✓ Поле is_metal_excluded уже существует в таблице calculation, пропускаем`);
      return;
    }
    ...
    // Добавляем поле
    const alterQuery = `ALTER TABLE ${schemaName}.calculation ADD COLUMN is_metal_excluded BOOLEAN DEFAULT FALSE`;
    await client.query(alterQuery);

    console.log(`   ✅ Поле is_metal_excluded успешно добавлено в таблицу calculation`);
...
// Запускаем скрипт
addIsMetalExcluded(userId).catch((error) => {
